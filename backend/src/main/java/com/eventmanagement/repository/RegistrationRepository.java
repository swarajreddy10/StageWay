...
public interface RegistrationRepository extends JpaRepository<Registration, Long> {
    ...
    @Query("select r.eventId, count(r) from Registration r where r.eventId in :eventIds and r.status in :statuses group by r.eventId")
    ...
    @Query(value = "select date(created_at) as day, count(*) from registrations where event_id = :eventId and created_at is not null group by date(created_at) order by day", nativeQuery = true)
    List<Object[]> countByEventIdGroupedByDate(@Param("eventId") Long eventId);

    @Query(value = "select extract(hour from created_at) as hour, count(*) from registrations where event_id = :eventId and created_at is not null group by extract(hour from created_at) order by hour", nativeQuery = true)
    ...
    @Query(value = "select count(*) from registrations where event_id in (:eventIds) and status in (:statuses)", nativeQuery = true)
